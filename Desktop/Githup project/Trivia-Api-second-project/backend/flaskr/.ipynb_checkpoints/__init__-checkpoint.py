...
def create_app(test_config=None):
    ...
    def add_question():
        ...
        try: 
            add_question=Question(question=input_question,answer=input_answer,category=input_category,difficulty=input_difficulty)
            add_question.insert()
            selection=Question.query.order_by(Question.id).all()
            current_questions=paginate_questions(request,selection)
            return jsonify({
    ...
    def unproccessed(error):
        return(
           jsonify({'success':False,'error':422, 'message':'request can not be proccessed'}),422
        )
